...
function normalizeBillShape(rawBill, table) {
  if (!rawBill || typeof rawBill !== 'object') {
    return null;
  }

  const items = normalizeItems(rawBill.items);
  const total = Number(computeTotal(items, rawBill.total).toFixed(2));
  ...
  return {
    ...
    items: items.map((item) => ({
      ...
      qty: item.qty,
      price: item.price,
      lineTotal: Number((item.subtotal ?? 0).toFixed(2))
    })),
    total,
    paymentMode: rawBill.paymentMode === 'split' ? 'split' : 'single',
    splitCount: rawBill.paymentMode === 'split'
      ? Math.max(2, Number.parseInt(rawBill.splitCount, 10) || 2)
      : null,
    updatedAt: rawBill.updatedAt || null
  };
}
...
exports.handler = async (event) => {
  ...
  if (event.httpMethod === 'GET') {
    ...
    const bill = normalizeBillShape(await loadBill(storage, key), table);
